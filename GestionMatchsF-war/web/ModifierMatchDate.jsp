        ...
        <form method ="get" action="gestionFed">
            ...
            <div class ='input2'>
                ...
                <select name="idM"class='custom-dropdown__select custom-dropdown__select--white'>
                    <% for (Matchs m : lesM ){%>
                    <option value="<%=m.getId()%>">Date : <%= m.getDateMatch().getDate()  %>/<%= m.getDateMatch().getMonth()+1%>/<%= m.getDateMatch().getYear()+1900 %> Equipes : <%=m.getEquipeUn().getNomequipe()  %> VS <%=m.getEquipeDeux().getNomequipe() %>   </option>
                    <%}%>
                </select> 
            </div>
                                </br>
